...
最終更新：2026-08-06（構成の簡素化とチェックイン追加まで）
...
## 3-b. 2026-08-06 の簡素化（0007）

オーナー指示で、使わない要素を削った。

| 変更 | 内容 |
|---|---|
| セッション | 参加者リストと種目を廃止。**レーキ額と参加人数だけ**にした。`players` / `session_players` テーブルと `game_type` は削除 |
| セッション一覧 | 誰が入力したか（`created_by` の表示名）を出す |
| 台帳 | 最上段に**残高を特大**で。明細に記入者を表示。狭い画面で潰れるため表を2段の行レイアウトへ |
| 固定費 | 家賃1件だけにして、その場で金額と計上日を編集できるようにした |
| TOP | タブ名を「積立」→「TOP」。最上段に**いま施設にいる人**、下に**直近の利用状況** |
| チェックイン | `check_ins` テーブルを追加。マストヘッド（sticky）のボタンで打刻し、全タブから押せる |
| ゲージ | ロックグラスに瓶からウイスキーが注がれる演出に変更（`WhiskyGauge`） |
| コピー | 「退出」→「ログアウト」、「今月ぶん」→「今月分」 |

`players` を消したので、メンバー画面から参加者マスタの節も外した。
