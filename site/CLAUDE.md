...
- **宴会の文面追記**：貸切「**25名様〜貸切可。フロア一体で最大28名まで**」、請求書「社内経理・後日精算用の請求書を発行（インボイス対応）」を追記。
  - ⚠️ **店主要確認の矛盾あり（未解決）**：今回の貸切文の「**フロア一体で最大28名**」が、既存の「宴会最大人数＝**最大36名**（総席数36席）」（上記2026-06-16に店主が28→36へ更新済み）と矛盾。どちらが正か店主確認のうえ片方に統一すること。site.json の `宴会情報.貸切` と `宴会情報.宴会最大人数` / `宴会情報.最大収容人数` / ヒーローバッジ「宴会 最大36名」/ SEO description を同期させる。
- **アクセス**：近隣ホテル導線を追加（`アクセス.ホテルから`＝「ホテルNo.1新居浜から徒歩約5〜7分」等）。⚠️ **概算・店主要確認**（ホテル名/徒歩分/道順は地図実測ではない。`_ホテル注意` 参照）。
- **喫煙表示**：`店舗情報.喫煙`＝「店内全席喫煙可（20歳未満入店不可）」を追加（Access に表示）。⚠️ **要確認**：改正健康増進法で「全席喫煙可＝喫煙可能店」扱い→店頭/サイトに標識・20歳未満入店不可が原則。宴会で20歳未満不可の影響あり。実態と異なれば店主が修正（分煙・時間帯制限など）。`_喫煙注意` 参照。
- **地図ピン**：緯度経度を実座標 **33.9654593,133.2795024**（Googleマップ登録の「しっぽり亭」）に更新、埋め込みURLもこの座標でピン表示に。表示位置は店主が最終確認。
...
## TODO（本人作業／次セッション候補）
- [ ] ⚠️ **宴会人数の矛盾を解消**：貸切「フロア一体で最大28名」 vs 既存「最大36名（36席）」。店主に正しい値を確認し、site.json の貸切/宴会最大人数/最大収容人数/ヒーローバッジ/SEO description を統一（詳細は「文面・スマホ・画像トリミング調整(2026-06-25)」節）
- [ ] ⚠️ **アクセスのホテル導線・喫煙表示を店主確認**：ホテル名/徒歩分（概算）と「全席喫煙可（20歳未満入店不可）」の法令対応（`_ホテル注意`/`_喫煙注意`）。地図ピンの実座標も最終確認
