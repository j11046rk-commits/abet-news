...
## ネット予約のインサイトをダッシュボードに追加（2026-08-11）
店主要望「HPの管理ダッシュボードでも予約システムへのアクセスのインサイトを見たい」に対応。

- **どこから取るか**：予約アプリ側の **`GET /api/insights/reservations`**（`shippori-reserve`）。
  `x-api-key: INSIGHTS_TOKEN` で守る。**返すのは集計だけ**で、氏名・電話番号・受付番号・メモは
  SQLの select にすら入れていない。ダッシュボードは静的HTML＋合言葉1つなので、そこへ
  個人情報を送る設計にしない、というのがこの機能の一番大事な決めごと。
- **取ってくる側**：`scripts/fetch-reserve.mjs` → `data/reserve.json`。
  書き出す前に「name/phone/memo…らしきキーが混ざっていないか」を自分でも検査して落とす
  （向こうの実装が将来変わっても、ここで止まる）。取得失敗時はコミット済みデータでビルド継続（GA4と同じ）。
- **表示**：`dashboard.astro` の「ネット予約システム」節。予約件数／ネット予約の件数と比率／
  ご来店予定の人数／取り消し率、日別の棒（濃い部分＝ネット予約）、予約の入り口、人数の分布、
  何日前に予約が入るか、来店の時間帯、曜日別。
- **必要なシークレット**：
  - Vercel（予約アプリ）… `INSIGHTS_TOKEN`（設定済み）
  - GitHub Actions（このリポジトリ）… `RESERVE_INSIGHTS_TOKEN`（**同じ値**。要設定）
  未設定でもビルドは通り、ダッシュボードは「まだ届いていません」と出るだけ。
- **【バグ修正】帯グラフがずっと空だった**：`.bar-fill` が `<span>` のままで `display:block` が
  無く、inline要素には `width` が効かないため**幅0pxで描画されていた**（流入チャネル・デバイスの
  帯グラフが公開時から全部空。トラックだけ見えていた）。`.bar-track`/`.bar-fill` に `display:block`
  を付けて解消。教訓：`<span>` に幅・高さを指定するときは display を必ず見る。
